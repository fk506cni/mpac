...
```{r}
f <-  "../../subjects/subjectslist.xlsx"
df <- read.xlsx(f)

df <- df %>% 
  mutate(age = Age_dummy,
         bw = BW_dummy,
         bh = BH_dummy)
```
...
t1 <- df %>%
  mutate(BMI = bw/(bh^2)*1e+4) %>% 
  select(Gender, age, bw,  bh, BMI, Study) %>%
...
t1 %>% as_gt() %>% as_latex() %>% as.character() %>% writeLines(con = "latex_out.tex")
