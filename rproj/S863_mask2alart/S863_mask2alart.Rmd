...
```{r}
f <- "../../mask/mask2alart.xlsx"
tt <-  read.xlsx(f) %>% 
  mutate(t = 24*60*Time) %>% 
  mutate(dev = case_when(
    Color == "black" ~ "vivoactive 5",
    Color == "white" ~ "Venu Sq 2",
    T ~ ""
  ))
tt
```
...
p_ma <- ggplot()+
  ...
  geom_point(data = tt,
             aes(x=dev,
                 y=t), color="red")+
  xlab("Device")+
  ylab("Time to alart (seconds)")+coord_flip(ylim = c(0, 240))
p_ma

ggsave2_tri(p_ma, wid = 12, hei = 10)
```
